...
public class Runner_Stanfard {
  ...
  public static class TokenizerMapper
    ...
    public void map(LongWritable key, Text value, Context context
    	...
    	//System.out.println("data is "+value.toString());
...
if(top.contains(id))
 ...
 public void reduce(Text key, Iterable<Text> values,
	 ...
	 for(Text value:values)
	 {
		...
		int output= SentimentAnalyzer.findSentiment(value.toString());
		positiveCount+=output;
		count++; 
		/*
	 String data[]=	value.toString().split(" ");
	...
	System.out.println("what!!"+(positiveCount-negativeCount));
	 ...
	 double average=positiveCount/count;
	 String sentiment="";
	 ...
	 context.write(new Text(key),new Text(sentiment+":"+average+":"+positiveCount+":"+count) );
  ...
  public static void main(String[] args) throws Exception {
   ...
   Job job = Job.getInstance(conf, "word count");
    job.setJarByClass(Runner_Stanfard.class);
    ...
    job1.setJarByClass(Runner_Stanfard.class);
  ...
  String data="";
   ...
   // DistributedCache.addCacheFile(new Path("hdfs://localhost:54310/words/positive-words.txt").toUri(), job2.getConfiguration());
    ...
    job2.setJarByClass(Runner_Stanfard.class);
    job2.setMapperClass(DataMapper.class);
    //job.setCombinerClass(IntSumReducer.class);
   job2.setReducerClass(DataReducer.class);
   job2.setPartitionerClass(SimplePartitioner.class);
